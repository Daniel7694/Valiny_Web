...
function Registro() {
  ...
  useEffect(() => {
    ...
    const fetchCursos = async () => {
      try {
        const response = await fetch('http://192.168.2.108:3000/api/cursos');
        if (!response.ok) {
  ...
  const createClave = async (clave) => {
    const response = await fetch('http://192.168.2.108:3000/api/clave/create', {
      method: 'POST',
  ...
  const createAdministrador = async (administrador) => {
    const response = await fetch('http://192.168.2.108:3000/api/administradores/create', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
